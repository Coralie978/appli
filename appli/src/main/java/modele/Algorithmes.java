...
public class Algorithmes {
    ...
    public static List<ResultatParcours> algoKMeilleuresSolutions(Scenario scenario, CarteFrance carte, int k) {
        ...
        // Construction sommets + indices (idem que ton algo)
        Map<String, Integer> sommetVersIndice = new HashMap<>();
        Map<Integer, String> indiceVersSommet = new HashMap<>();
        int index = 0;
        for (Vente v : ventes) {
        ...
        // Classe pour garder état partiel
        class Etat implements Comparable<Etat> {
            List<Integer> parcoursSommets; // indices des sommets visités
            int[] degresEntrants;
        ...
        // Initialisation : sommets avec degré entrant 0
        Set<Integer> disponiblesInit = new HashSet<>();
        for (int i = 0; i < index; i++) {
            if (degresEntrantsInit[i] == 0) disponiblesInit.add(i);
        }

        // On commence par "Velizy" (même logique que toi)
        // Trouver les sommets dispo correspondant à Velizy+ ou Velizy- ?
        // Pour simplifier, on démarre sans sommets visités, ville actuelle Velizy, degresEntrants init.
        ...
        while (!pq.isEmpty() && resultats.size() < k) {
            ...
            // Calculer les sommets disponibles actuels (deg entrants == 0, non visités)
            Set<Integer> disponibles = new HashSet<>();
            for (int i = 0; i < index; i++) {
            ...
            if (disponibles.isEmpty()) {
                ...
                for (int s : courant.parcoursSommets) {
                    ...
                    String ville = sommet.substring(0, sommet.length() - 1);
                    if (!parcoursVilles.get(parcoursVilles.size() -1).equals(ville)) {
            ...
            // Sinon, étendre avec tous les choix possibles
            for (int suivant : disponibles) {
